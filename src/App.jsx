...
export default function MarketplaceLanding() {
  ...
  const placeOrder = () => {
    ...
    const order = { id: `ORD-${1000 + orders.length + 1}`, items, total, placedAt: new Date() };
    setOrders((o) => [order, ...o]);
    setCart([]);
    openOrders();
  };
  ...
  return (
    <div style={{ background: PALETTE.bg, color: PALETTE.ink, fontFamily: "Inter, sans-serif" }} className="min-h-screen">
        ...
        <div style={{ background: PALETTE.navy }}>
          <div className="max-w-[1500px] mx-auto px-3 py-2 flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center gap-1 flex-shrink-0">
              ...
              >
                <span className="text-[11px] text-white/70 leading-tight">Hello, sign in</span>
                <span className="text-xs font-bold text-white leading-tight">Accounts & Lists</span>
              </button>
      ...
      <CartDrawer
        ...
        onCheckout={() => { setCartOpen(false); setCheckoutIntent(true); setAuthOpen(true); }}
      />
    </div>
  );
}
