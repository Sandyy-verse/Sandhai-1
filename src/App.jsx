...
function CartDrawer({ open, cart, products, onClose, onChangeQty, onRemove, onCheckout, onBrowse }) {
  if (!open) return null;
...
function AdminPanel({ onBack, allProducts }) {
  const [tab, setTab] = useState("products"); // products | orders
  const [adminProducts, setAdminProducts] = useState([]);
  const [allOrders, setAllOrders] = useState([]);
  const [form, setForm] = useState({
    name: "", price: "", mrp: "", unit: "piece", laneKey: "A", seller: "", town: "",
  });
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (!form.name.trim() || !form.price || !imageFile) {
      setErrorMsg("Name, price, and an image are required.");
      return;
    }
    setStatus("uploading");
    setErrorMsg("");
    try {
      ...
      await addDoc(collection(db, "products"), {
        ...
        unit: form.unit,
        seller: form.seller.trim() || "Independent seller",
        town: form.town.trim() || "Tamil Nadu",
        rating: 4.5,
        reviews: 0,
        prime: false,
        img: imgUrl,
      ...
      setForm({ name: "", price: "", mrp: "", unit: "piece", laneKey: "A", seller: "", town: "" });
      setImageFile(null);
      ...
      setStatus("error");
      setErrorMsg(err.message || "Upload failed. Check Storage rules.");
    }
  };
  ...
  return (
    <section className="max-w-[1100px] mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <button onClick={onBack} className="text-xs flex items-center gap-1 mb-2" style={{ color: PALETTE.link }}>
            <ArrowLeft size={14} /> Back to store
          </button>
          <h1 className="text-2xl font-bold" style={{ color: PALETTE.ink }}>Admin panel</h1>
        </div>
        <div className="flex gap-2">
          {["products", "orders"].map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className="px-4 py-2 rounded-md text-sm font-semibold border capitalize"
              style={{
                borderColor: tab === t ? PALETTE.navy : PALETTE.border,
                background: tab === t ? PALETTE.navy : "transparent",
      ...
      {tab === "products" && (
        <div className="grid md:grid-cols-2 gap-8">
          <form onSubmit={submit} className="rounded-2xl border p-5 flex flex-col gap-3" style={{ borderColor: PALETTE.border }}>
            <h2 className="font-bold text-sm" style={{ color: PALETTE.ink }}>Add a new listing</h2>

            <Field label="Product name">
              <input value={form.name} onChange={update("name")} className="w-full bg-transparent outline-none text-sm" placeholder="e.g. Coimbatore filter coffee" />
            </Field>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Price (₹)">
                <input type="number" value={form.price} onChange={update("price")} className="w-full bg-transparent outline-none text-sm" placeholder="199" />
              </Field>
              <Field label="MRP (optional)">
                <input type="number" value={form.mrp} onChange={update("mrp")} className="w-full bg-transparent outline-none text-sm" placeholder="249" />
              </Field>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Unit">
                <input value={form.unit} onChange={update("unit")} className="w-full bg-transparent outline-none text-sm" placeholder="kg / piece / set" />
              </Field>
              <label className="block">
                <span className="text-xs font-semibold opacity-70" style={{ color: PALETTE.ink }}>Category</span>
                <div className="mt-1 px-3 py-2 rounded-lg border" style={{ borderColor: PALETTE.border }}>
                  <select value={form.laneKey} onChange={update("laneKey")} className="w-full bg-transparent outline-none text-sm">
                    {LANES.map((l) => (
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Seller name">
                <input value={form.seller} onChange={update("seller")} className="w-full bg-transparent outline-none text-sm" placeholder="Stall / business name" />
              </Field>
              <Field label="Town">
                <input value={form.town} onChange={update("town")} className="w-full bg-transparent outline-none text-sm" placeholder="e.g. Erode" />
              </Field>
            </div>

            <label className="block">
              <span className="text-xs font-semibold opacity-70" style={{ color: PALETTE.ink }}>Product image</span>
              <div className="mt-1 flex items-center gap-3">
                <input type="file" accept="image/*" onChange={handleFile} className="text-xs" />
                {imagePreview && (
                  <img src={imagePreview} alt="preview" className="w-14 h-14 rounded-lg object-cover border" style={{ borderColor: PALETTE.border }} />
                )}
              </div>
            </label>

            {errorMsg && <p className="text-xs" style={{ color: PALETTE.price }}>{errorMsg}</p>}

            <button
              type="submit"
              disabled={status === "uploading"}
              className="mt-2 w-full py-2.5 rounded-md font-semibold text-sm flex items-center justify-center gap-2"
              style={{ background: PALETTE.navy, color: "#fff" }}
            >
              {status === "uploading" ? (<><Loader2 size={16} className="animate-spin" /> Uploading</>) : status === "done" ? "Listed!" : "Publish listing"}
            </button>
          </form>

          <div className="rounded-2xl border p-5" style={{ borderColor: PALETTE.border }}>
            <h2 className="font-bold text-sm mb-3" style={{ color: PALETTE.ink }}>
              Your listings ({adminProducts.length})
            </h2>
            <div className="flex flex-col gap-3 max-h-[520px] overflow-y-auto pr-1">
              {adminProducts.length === 0 && (
                <p className="text-xs opacity-50">Nothing uploaded yet — add your first product on the left.</p>
              )}
              {adminProducts.map((p) => (
                <div key={p.id} className="flex items-center gap-3 border rounded-xl p-2" style={{ borderColor: PALETTE.border }}>
                  <img src={p.img} alt={p.name} className="w-12 h-12 rounded-lg object-cover flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate" style={{ color: PALETTE.ink }}>{p.name}</p>
                    <p className="text-xs opacity-60">₹{p.price} · {p.seller}</p>
                  </div>
                  <button onClick={() => removeProduct(p.id)} className="p-1.5 rounded-md hover:bg-black/5 flex-shrink-0" aria-label="Delete listing">
                    <X size={16} color={PALETTE.price} />
                  </button>
                </div>
              ))}
            </div>
      ...
      {tab === "orders" && (
        <div className="flex flex-col gap-3">
          <p className="text-xs opacity-60 mb-1">{allOrders.length} order{allOrders.length === 1 ? "" : "s"} across all buyers</p>
          {allOrders.map((o) => (
            <div key={o.id} className="rounded-xl border p-4" style={{ borderColor: PALETTE.border }}>
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-bold" style={{ color: PALETTE.ink }}>{o.orderRef || o.id}</p>
              ...
              <div className="flex flex-col gap-1">
                {o.items?.map((i, idx) => (
                  <div key={idx} className="flex justify-between text-xs opacity-75">
                    <span>{i.name} × {i.qty}</span>
                    <span>₹{(i.qty * i.price).toLocaleString()}</span>
                  </div>
                ))}
              </div>
              <div className="mt-2 pt-2 border-t flex justify-between text-sm font-bold" style={{ borderColor: PALETTE.border, color: PALETTE.ink }}>
...
export default function MarketplaceLanding() {
  ...
  const [page, setPage] = useState("home"); // home | browse | orders | admin
  ...
  // Merge built-in catalog with anything published from the Admin panel.
  const allProducts = [...PRODUCTS, ...adminProducts];

  // Subscribe to admin-published products (public catalog, so everyone sees new listings).
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "products"), (snap) => {
      setAdminProducts(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
    });
  ...
  const openAdmin = () => { setPage("admin"); window.scrollTo(0, 0); };
  ...
  const browseProducts =
    browseFilter?.type === "lane"
      ? allProducts.filter((p) => p.laneKey === browseFilter.key)
  ...
  return (
    <div style={{ background: PALETTE.bg, color: PALETTE.ink, fontFamily: "Inter, sans-serif" }} className="min-h-screen">
      ...
      <header className="sticky top-0 z-40">
        <div style={{ background: PALETTE.navy }}>
          <div className="max-w-[1500px] mx-auto px-3 py-2 flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center gap-1 flex-shrink-0">
              {isAdmin && (
                <button onClick={openAdmin} className="flex flex-col px-2 py-1 rounded-sm border border-transparent hover:border-white text-left">
                  <span className="text-[11px] text-white/70 leading-tight">Manage</span>
                  <span className="text-xs font-bold text-white leading-tight">Admin panel</span>
                </button>
              )}
              <button
        ...
        {menuOpen && (
          <div className="md:hidden px-5 py-4 flex flex-col gap-3" style={{ background: PALETTE.navyLight }}>
            ...
            {isAdmin && (
              <button onClick={() => { openAdmin(); setMenuOpen(false); }} className="text-left py-1 text-sm font-medium text-white/90">
                Admin panel
              </button>
            )}
            {currentUser ? (
      ...
      {page === "admin" && isAdmin && (
        <AdminPanel onBack={goHome} allProducts={allProducts} />
      )}
